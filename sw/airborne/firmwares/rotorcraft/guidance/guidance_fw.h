 ...
 * range: 0 : #MAX_PPRZ
 */
extern int16_t guidance_fw_nominal_throttle;

extern int32_t guidance_fw_kp; ///< vertical control P-gain
extern int32_t guidance_fw_kd; ///< vertical control D-gain
extern int32_t guidance_fw_ki; ///< vertical control I-gain

extern void guidance_fw_init(void);
extern void guidance_fw_read_rc(void);
extern void guidance_fw_mode_changed(uint8_t new_mode);
extern void guidance_fw_notify_in_flight(bool_t in_flight);
extern void guidance_fw_run(bool_t in_flight);

#define guidance_fw_SetKi(_val) {			\
    guidance_fw_ki = _val;				\
    guidance_fw_z_sum_err = 0;			\
  }

#define guidance_fw_SetNominalHoverThrottle(_throttle) { \
    guidance_fw_nominal_throttle = _throttle;            \
    Bound(guidance_fw_nominal_throttle, 0.1*MAX_PPRZ, 0.9*MAX_PPRZ);  \
  }
